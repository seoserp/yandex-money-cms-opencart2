#Y.CMS для Opencart 2.3.x от Яндекс.Кассы
...
###Совместимость с CMS Opencart 2.0.x - 2.2.x
Для CMS Opencart 2.0.x - 2.2.x нами разработан универсальный модуль, который можно [загрузить здесь](https://github.com/yandex-money/yandex-money-cms-opencart2/tree/master).

###Установка
Инструкция размещена на [странице сервиса Яндекс.Касса](https://kassa.yandex.ru/files/manual_y.cms_opencart2.pdf).
> Внимание! В версии модуля 2.0.0 произошли изменения адресов для взаимодействия со внешними сервисами Яндекса.

> Для работы с Яндекс.Маркетом по программе "Заказы в Маркете" необходимо добавить строки ниже в файл `.htaccess` корневого каталога сайта.
> ```
RewriteBase /
RewriteRule ^yandexbuy/([^?]*)$ index.php?route=extension/yandexbuy/$1 [L,QSA]
> ```
